...
- **`/starboard lookback`** — Scans the most recent messages of a starboard's watch channel (200 by default, up to 1000 — or a longer stretch back to a specific date, if you ask for that instead) for ones that already qualify but haven't been picked up yet — handy right after creating a new starboard, or to catch up on messages missed while the bot was offline. In Reactions mode it re-checks existing reactions; in Buttons mode it adds a vote button to any matching message that doesn't already have one. You can also pick a different content-type filter just for that scan (e.g. only images), without changing the starboard's saved settings. If a handful of messages fail to check (a temporary hiccup), the scan keeps going and tells you how many it had to skip — you can just run it again to pick those up.
