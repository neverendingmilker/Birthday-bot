...
- `/starboard lookback name:<...> [limit:1-1000, default 200] [since_year_start:true|false] [content_type]` — scans messages of the starboard's watch channel for ones that already qualify. By default it scans the most recent `limit` messages; passing `since_year_start:true` instead scans everything back to **midnight, January 1st of the current year** (in the bot's configured timezone — the same `TZ` used by birthdays/incident), ignoring `limit` (capped internally at 20,000 messages as a safety ceiling). The optional `content_type` lets you check a different kind of message than what the starboard is normally configured for, just for this one scan — e.g. a starboard that's normally "Any message" can be scanned with `content_type:Images only` to only backfill images, without touching the starboard's saved filter; if omitted, the starboard's own configured content-type filter is used, same as always. Useful right after creating a starboard (to backfill already-popular older messages) or to catch up on messages sent while the bot was offline. In **Reactions** mode it re-runs the normal counting/sync logic against each message's current reactions (so it can also *remove* a post if a message no longer qualifies). In **Buttons** mode it just adds a vote button to any matching message that doesn't already have one — it never invents past votes, since buttons only count clicks made from here on. This can take a little while on a busy channel (fetching message history and, in Reactions mode, the users behind each reaction — a full year's worth can take several minutes), so the bot replies once scanning finishes rather than instantly.
